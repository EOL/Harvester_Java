...
//                            file2 = new File(version2.getLocation().getPath() + "/" + (archiveFile2.getTitle()));
    ...
    public void setOccurrenceOfChangedMeasurements(Archive updatedArchive) {
        ...
        if (measurementFile != null) {
            ...
            for (Record record : measurementFile) {
                if (measurementIds.contains(record.value(DwcTerm.measurementID)) && !occurrenceIds.contains(record.value(CommonTerms.occurrenceIDTerm))) {
                    ModelsIds.getModelsIds().addOccurrenceId(record.value(CommonTerms.occurrenceIDTerm));
                }
            }

            for (Record record : occurrenceFile) {
                if (!notUpdatedOccurrenceIds.contains(record.value(CommonTerms.occurrenceIDTerm)) &&
                        occurrenceIds.contains(record.value(CommonTerms.occurrenceIDTerm))) {

                    actionFile.writeLineToActionFile(occurrenceFile, record.value(CommonTerms.occurrenceIDTerm), occurrenceFile.getFieldsTerminatedBy(), actionFile.getActionIndicator(ActionFile.Action.Unchanged));
    ...
    public void setOccurrenceOfChangedAssociations(Archive updatedArchive) {
        ...
        if (associationFile != null) {
            ...
            for (Record record : associationFile) {
                if (associationIds.contains(record.value(CommonTerms.associationIDTerm)) && !occurrenceIds.contains(record.value(CommonTerms.occurrenceIDTerm))) {
                    ModelsIds.getModelsIds().addOccurrenceId(record.value(CommonTerms.occurrenceIDTerm));
                }
            }
            ...
            for (StarRecord starRecord : updatedArchive) {
                if (starRecord.extension(CommonTerms.occurrenceTerm) != null) {
                    for (Record record : starRecord.extension(CommonTerms.occurrenceTerm)) {
                        if (occurrenceIds.contains(record.value(CommonTerms.occurrenceIDTerm)) && !taxaIds.contains(starRecord.core().id())) {
                            ModelsIds.getModelsIds().addTaxaId(starRecord.core().id());
                            actionFile.writeLineToActionFile(taxaFile, starRecord.core().id(), taxaFile.getFieldsTerminatedBy(), actionFile.getActionIndicator(ActionFile.Action.Unchanged));
                            writeRecordToArchiveFile(starRecord.core(), taxaFile);
                        }
