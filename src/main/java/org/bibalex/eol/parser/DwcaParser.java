...
public class DwcaParser {
    ...
    public void prepareNodesRecord(int resourceId) {
        ...
        if(parentFormat){
            System.out.println("Parent Format");
            ParentFormat format = new ParentFormat(resourceId);
            format.insertTaxontoMysql(dwca, entityManager);
        }
        else {
...
//            if(rec.core().value(DwcTerm.taxonRank) != null && rec.core().value(DwcTerm.taxonRank).contains("domain"))
//                System.out.println(rec.core().value(DwcTerm.taxonID)+" "+rec.core().value(DwcTerm.parentNameUsageID));
            if(rec.core().value(DwcTerm.taxonID).equals("93302"))
                System.out.println(rec.core().value(DwcTerm.taxonRank));
        }
